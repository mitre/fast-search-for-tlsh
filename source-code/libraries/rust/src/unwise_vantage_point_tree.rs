// A derivative of the vantage-point tree class, but with changes to emulate a cutoff of 20
// on total distance's triangle inequality violation.

use crate::common::TlshDigest;
...
fn total_distance(x: &TlshDigest, y: &TlshDigest) -> i32 {
    TlshDigest::distance_headers(x, y) + TlshDigest::distance_bodies(x, y)
}
...
impl VPTreeImpl {
    ...
    fn _insert(node: Option<Box<VPNode>>, point: TlshDigest) -> Option<Box<VPNode>> {
        match node {
            ...
            Some(mut n) => {
                let dist = total_distance(&n.point, &point);
                if dist <= n.threshold {
    ...
    fn _build_tree(nodes: &mut [TlshDigest]) -> Option<Box<VPNode>> {
        ...
        let mut sampled_distances: Vec<(i32, usize)> = (1..=sample_size)
            .map(|i| (total_distance(&node.point, &nodes[i]), i))
            .collect();
        ...
        while left <= right {
            while left <= right && total_distance(&node.point, &nodes[left]) < estimated_median {
                left += 1;
            }
            while left <= right && total_distance(&node.point, &nodes[right]) >= estimated_median {
                right -= 1;
            }
            if left < right {
    ...
    pub fn range_query(
        ...
        let bounds_adjusted_radius = radius + max_violation;
        ...
        while let Some(node) = stack.pop() {
            if let Some(n) = node {
                ...
                let distance = total_distance(&n.point, query_point);

                if distance <= radius {
                    results.push(n.point.clone());
                }

                if distance - bounds_adjusted_radius <= n.threshold {
                    stack.push(&n.left);
                }
                if distance + bounds_adjusted_radius >= n.threshold {
                    stack.push(&n.right);
...
pub struct UnwiseVantagePointTree {
    tree: VPTreeImpl,
}

impl UnwiseVantagePointTree {
    pub fn new(nodes: Vec<TlshDigest>) -> Self {
        let mut tree = VPTreeImpl::new();
        tree.bulk_insert(nodes);
        UnwiseVantagePointTree { tree }
    }
    ...
    pub fn query(
        ...
        self.tree.range_query(query_point, radius, max_violation)
    }
}
